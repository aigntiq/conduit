...
| [Microsoft Outlook](connectors/microsoft-outlook/README.md) | Microsoft OAuth (a user, or an app on one mailbox) | send email, create draft, reply (all), forward, search messages, get message, download attachment, folders (options), move message, mark/flag/categorize, delete message, new email (webhook trigger) |
| [Microsoft Calendar](connectors/microsoft-calendar/README.md) | Microsoft OAuth (a user, or an app on one mailbox) | calendars (options), search events, get event, create event (attendees, Teams meeting), update event, delete event, respond to invitation, find meeting times, free/busy, event created/changed/deleted (webhook trigger) |
...
`__tests__/support/stub.ts`. `renderPoll()` and `renderWebhook()` from `__tests__/support/triggers.ts`
render a trigger's templates (a poll against a canned answer; a webhook's
subscription and a delivery), since the trigger runtime isn't released yet.
