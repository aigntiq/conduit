#!/usr/bin/env node
/**
 * Apply the standard `main` protection to a repo — branch rules as code.
 *
 ...
 *   node scripts/apply-branch-protection.mjs aigntiq/conduit --checks "test (ubuntu-latest, 22); verify-pack"
 *
 * Requirements: `gh` CLI authenticated (`gh auth login`) with admin on the repo.
 */
import { spawnSync } from 'node:child_process';

const RULESET_NAME = 'aigntiq: protect main';
...
const ruleset = {
    ...
    rules,
    // Empty bypass list: not even admins skip the PR process. Add actor IDs here if
